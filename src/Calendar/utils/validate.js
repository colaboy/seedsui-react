...
function validate(jumpDate, { min, max, onError }) {
  ...
  // 如果最小值大于等于最大值则无法生效
  if (min instanceof Date && max instanceof Date && DateUtil.compareDate(min, max) >= 1) {
    let errMsg =
  ...
  // 小于最小值
  if (min instanceof Date && DateUtil.compareDate(jumpDate, min) === -1) {
  ...
  // 大于最大值
  if (max instanceof Date && DateUtil.compareDate(jumpDate, max) === 1) {
