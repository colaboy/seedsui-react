  ...
  async function handleChoose(event) {
    ...
    let chooseImageParams = {
      ...
      success: async (res) => {
        ...
        // 异步上传，需要将localId转成base64后显示(客户端同步上传已经改为超时机制)
        // let base64List = []
        // if (async) {
        ...
        let currentList = localIds.map((localId, index) => {
          return {
            ...
            sceneCheck: sceneCheckMap[localId] || null
          }
        })
