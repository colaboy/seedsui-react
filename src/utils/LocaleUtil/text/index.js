...
export default function (remark, key, variable, dangerouslyHTML) {
  let localeData = { ...(window.seedsLocaleData || {}), ...(window.localeData || {}) }
  // 如果没有参数, 则返回国际化数据
  ...
  if (key && localeData) {
    ...
    if (!value || typeof value !== 'string') {
      return Interpolate.toNode(remark, dangerouslyHTML)
    }
  ...
  // 没有国际化的值, 则渲染remark
  if (remark) return Interpolate.toNode(remark, dangerouslyHTML)
  return ''
}
