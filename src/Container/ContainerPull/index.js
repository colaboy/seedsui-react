  ...
  (
    {
      onTopRefresh,
      onBottomRefresh,
      refreshing, // true:正在刷新; false:刷新完成; 其它:不执行刷新
      children,
      ...others
    },
    ref
    ...
    const refreshingRef = useRef({ current: refreshing })
    ...
    // 初始化
    if (refreshingRef.current === 0) refreshingRef.current = false
    else if (refreshingRef.current === 1) refreshingRef.current = true
    useEffect(() => {
    ...
    function bottomRefresh() {
      ...
      if (refreshingRef.current !== true && refreshingRef.current !== false) {
        refreshed()
        return
      }
      // 正在刷新不触发底部刷新
      if (refreshingRef.current === true) return
      // 刷新完成触发底部刷新
    ...
    useEffect(() => {
      ...
      if (refreshingRef.current === false) {
        ...
        // 底部刷新(1.无onBottomRefresh不触发底部刷新; 2.容器隐藏(className=hide)不触发底部刷新)
        if (!onBottomRefresh || !rootRef.current || rootRef.current.classList?.contains('hide')) {
          return
        }
