...
let Body = function (params) {
  ...
  let defaults = {
    ...
    onTopHid(s)// 头部隐藏动画结束
    onTransitionEnd:function(s)// 头部动画结束
  ...
  s.container =
    typeof s.params.container === 'string'
      ...
      : s.params.container
  if (!s.container) {
    console.log('SeedsUI Error : Body container不存在，请检查页面中是否有此元素')
    return
  }
  // topContainer
  s.topContainer =
  ...
  // 头部刷新, 刷新中时将不允许刷新
  s.topRefresh = async function () {
    if (typeof s.params.onTopRefresh !== 'function') return
